...
if(filename~=0)
    handles.file=imread([filepath filename]);
end
guidata(hObject, handles);
...
handles.qim=str2double(get(handles.qim,'string'));
guidata(hObject, handles);
...
handles.skalar=str2double(get(handles.skalar,'String'));
guidata(hObject, handles);
...
handles.klucz=str2double(get(handles.klucz,'String'));
